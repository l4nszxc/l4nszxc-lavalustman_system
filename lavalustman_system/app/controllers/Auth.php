...
class Auth extends Controller {
    ...
    public function login() {
        if($this->form_validation->submitted()) {
            ...
            if(empty($data)) {
                ...
                redirect('home'); // Redirect to home on successful login
            }
        } 
        $this->call->view('auth/login');
    }
    ...
    public function register() {
            ...
            $this->form_validation
                ...
                if($this->form_validation->run()) {
                    if($this->lauth->register($username, $email, $this->io->post('password'), $email_token)) {
                        ...
                        set_flash_alert('success', 'Registration successful! Please verify your email.');
                        redirect('auth/login'); // Redirect to login page after registration
                    } else {
        ...
        if($this->form_validation->submitted()) {
			...
			if($this->form_validation->run()) {
				if($token = $this->lauth->registervf($email)) {
					$this->send_email_token_to_email($email, $token);
    ...
    private function send_email_token_to_email($email, $token) {
		$template = file_get_contents(ROOT_DIR.PUBLIC_DIR.'/templates/Verify.html');
		$search = array('{token}', '{base_url}');
		$replace = array($token, base_url());
		$template = str_replace($search, $replace, $template);
		$this->email->recipient($email);
		$this->email->subject('LavaLust Verify Email'); //change based on subject
		$this->email->sender('abaskien@email.com'); //change based on sender email
		$this->email->reply_to('abaskien@email.com'); // change based on sender email
    ...
    public function verifying() {
			...
			if(isset($token) && !empty($token)) {
				...
				$this->form_validation
					...
					->name('re_password')
						...
						if($this->form_validation->run()) {
							if($this->lauth->verification_now($token, $password)) {
								set_flash_alert('success', 'email was successfully updated.');
							} else {
								set_flash_alert('danger', config_item('SQLError'));
							}
						} else {
    	...
    	redirect('auth/verified/?token='.$token);
        } else {
             $token = $_GET['token'] ?? '';
            if(! $this->lauth->get_verification_token($token) && (! empty($token) || ! isset($token))) {
                set_flash_alert('danger', 'Invalid email reset token.');
            }
            $this->call->view('auth/verified');
        }
            ...
            if(isset($token) && !empty($token)) {
                ...
                $this->form_validation
                    ...
                    ->name('re_password')
                        ...
                        ->matches('password', 'Passwords did not match.'); // Added missing matches validation
